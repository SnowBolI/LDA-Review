...
def load_data(app_name, force_refresh=False):
    # Check if data exists and handle refresh
    path = f"data/data_per_app/{app_name}.csv"
    if os.path.exists(path):
        if force_refresh:
            update_progress(1, f"Data {app_name} ditemukan, menghapus data lama...", app_name)
            try:
                os.remove(path)
                update_progress(2, f"Data lama {app_name} berhasil dihapus", app_name)
                print(f"[LDA] Existing data for {app_name} deleted for refresh")
            except Exception as e:
                error_msg = f"Gagal menghapus data lama {app_name}: {str(e)}"
            ...
            try:
                ...
                print(f"[DEBUG] Loaded existing data for {app_name}:")
    ...
    # If no data exists or force_refresh is True, scrape new data
    update_progress(3, f"Data tidak ditemukan atau refresh diminta, mulai scraping {app_name}...", app_name)
    try:
        ...
        update_progress(10, f"Data {app_name} berhasil disimpan ({len(df)} reviews)", app_name)
        
        # Debug the loaded data
        print(f"[DEBUG] Loaded fresh data for {app_name}:")
        debug_data_structure(app_name)
